...
import com.mojise.library.ax_designsystem.ext.dp
import java.io.Serializable

private data class ViewBinding(
    val model: AxBasicDialogFragment.UiModel,
...
class AxBasicDialogFragment : DialogFragment() {
    ...
    ): View = inflater.inflate(R.layout.dialog_ax_modal_loading, container, false).also {
        binding = ViewBinding(
            model = model,
    ...
    fun from(supportFragmentManager: FragmentManager): TwoButtonRequestManager {
        return TwoButtonRequestManager(this, supportFragmentManager)
    }
...
//    }
    ...
    class OneButtonRequestManager(
        ...
        fun confirmClicked(isAutoDismiss: Boolean = true, action: () -> Unit) = apply {
            dialog.positiveButtonClickedAction = ClickAction(
                isAutoDismiss = isAutoDismiss,
                action = action
            )
        }

        fun show() = dialog.show(supportFragmentManager, TAG)
    }
    ...
    class TwoButtonRequestManager(
        ...
        fun negativeClicked(isAutoDismiss: Boolean = true, action: () -> Unit) = apply {
            dialog.negativeButtonClickedAction = ClickAction(
                isAutoDismiss = isAutoDismiss,
                action = action
            )
        }
    ...
    companion object {
        ...
        fun newInstance(model: UiModel) = AxBasicDialogFragment().also {
